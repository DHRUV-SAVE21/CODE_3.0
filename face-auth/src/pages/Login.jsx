...
import { faceLogin } from "../api/client";
...
function Login() {
  ...
  const scanFace = async () => {
    ...
    const faceApiInterval = setInterval(async () => {
      ...
      if (results.length > 0 && tempAccount.id === results[0].label) {
        ...
        try {
          await faceLogin(tempAccount.id, true);
        } catch (err) {
          console.error("Failed to notify backend about face login", err);
        }
      } else {
  ...
  async function loadLabeledImages() {
    ...
    try {
      let imgSource;
      if (tempAccount?.type === "CUSTOM") {
        if (tempAccount.picture && tempAccount.picture.startsWith("data:")) {
          imgSource = tempAccount.picture;
        } else if (tempAccount.backendPictureUrl) {
          imgSource = tempAccount.backendPictureUrl;
        } else {
          imgSource = tempAccount.picture;
        }
